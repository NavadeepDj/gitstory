...
export default function TopicsSlide({ data, isActive }: SlideProps) {
    ...
    const maxTopics = isMobile ? 8 : 15; // Fewer topics on mobile
    const uniqueTopics = Array.from(new Set(allTopics)).slice(0, maxTopics);
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full p-4 md:p-8 bg-background text-foreground relative overflow-hidden">
            ...
            <motion.h2
                ...
                className="text-2xl md:text-4xl font-bold mb-6 md:mb-12 text-center z-10 font-serif"
            >
                {title}
            </motion.h2>

            <div className="flex flex-wrap justify-center gap-2 md:gap-4 max-w-md z-10 content-center perspective-1000 px-2">
                {displayItems.map((topic, index) => {
                    // Randomize size slightly for "cloud" feel - smaller on mobile
                    const size = isMobile
                        ? (index % 3 === 0 ? "text-base" : index % 2 === 0 ? "text-sm" : "text-sm")
                    ...
                    return (
                        <motion.span
                            ...
                            animate={isActive ? {
                                scale: 1,
                                opacity: 1,
                                // Only enable floating animation on desktop for performance
                                ...(isMobile ? {} : { y: [0, -10, 0] })
                            } : {}}
                            transition={{
                            ...
                            className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-primary/10 text-primary border border-primary/20 backdrop-blur-md ${size} ${weight}`}
                            style={{
